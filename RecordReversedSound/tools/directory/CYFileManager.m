...
+ (instancetype)defaultManager {
    static CYFileManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager                                                     = [[CYFileManager alloc] init];
    });
    return manager;
}

- (NSString *)createAudioFilePathInDirectory:(NSString *)directory fileName:(NSString *)fileName {
    ...
    return [NSString stringWithFormat:@"%@/%@(%ld).m4a", docPath, fileName, ((unsigned long)[filelist count])];
}
...
- (NSArray *)filesInDirectory:(NSString *)directory sortedByAge:(NSComparisonResult)sort {
    ...
    if (error || filelist.count < 1) {
        NSLog(@"filesInDirectory error : %@", error);
        return  @[];
    }
...
- (unsigned long long)sizeOfAllFilesInDirectory:(NSString *)directory {
    // get documents
    NSString *docPath = [NSString stringWithFormat:@"%@/%@", [self documentsDirectory], directory];
