...
@implementation CYDevice : NSObject

//device_token, device_model, device_disk_space, device_battery, app_version, app_name, app_build
+ (NSDictionary *)specs {
    return @{
             @"model"               : [self model],
             @"disk_space"          : [NSNumber numberWithLongLong:[CYDisk freeDiskSpaceMB]],
...
+ (NSString *)commonName:(NSString *)machineName {
    ...
    NSArray *device = [[CYDevice deviceModelDataForMachineIDs] objectForKey:machineName];
    if (device) {
...
- (NSDictionary *)deviceSpecs {
    NSMutableDictionary *mutableSpecs                       = [[CYDevice specs] mutableCopy];
    [mutableSpecs removeObjectForKey:@"model"];
    [mutableSpecs removeObjectForKey:@"uudd"];
    NSString *network                                       =([CYDevice isUsingLTE]) ? @"LTE" : @"Wifi";
